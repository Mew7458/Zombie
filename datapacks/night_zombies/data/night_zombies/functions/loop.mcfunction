...
execute as @a[scores={nz_init=0}] at @s run function night_zombies:pure/give_beacon
scoreboard players set @a[scores={nz_init=0}] nz_init 1
title @a actionbar {"text":"Night Zombies running / 特殊怪系统运行中","color":"green"}
execute if score #daytime nz_time matches 13000..23000 if score #spawn nz_timer matches 2.. run function night_zombies:spawning/night_spawn
execute if score #spawn nz_timer matches 2.. run scoreboard players set #spawn nz_timer 0
